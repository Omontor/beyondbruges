...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.prizes.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="thumb_image">{{ trans('cruds.prize.fields.thumb_image') }}</label>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.prize.fields.thumb_image_helper') }}</span>
            </div>
            <div class="form-group">
                <label for="gallery">{{ trans('cruds.prize.fields.gallery') }}</label>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.prize.fields.gallery_helper') }}</span>
            </div>
            <div class="form-group">
                <label class="required" for="cost">{{ trans('cruds.prize.fields.cost') }}</label>
                <input class="form-control {{ $errors->has('cost') ? 'is-invalid' : '' }}" type="number" name="cost" id="cost" value="{{ old('cost', '') }}" step="1" required>
                @if($errors->has('cost'))
                    <span class="text-danger">{{ $errors->first('cost') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.prize.fields.cost_helper') }}</span>
            </div>
            <div class="form-group">
                <label class="required" for="quantity">{{ trans('cruds.prize.fields.quantity') }}</label>
...
<script>
    ...
    init: function () {
@if(isset($prize) && $prize->thumb_image)
      var file = {!! json_encode($prize->thumb_image) !!}
...
Dropzone.options.galleryDropzone = {
    ...
    init: function () {
@if(isset($prize) && $prize->gallery)
      var files = {!! json_encode($prize->gallery) !!}
          for (var i in files) {
          var file = files[i]
          this.options.addedfile.call(this, file)
          this.options.thumbnail.call(this, file, file.preview)
          ...
          return loader.file
            .then(function (file) {
              return new Promise(function(resolve, reject) {
                ...
                xhr.open('POST', '{{ route('admin.prizes.storeCKEditorImages') }}', true);
                ...
                data.append('crud_id', '{{ $prize->id ?? 0 }}');
                xhr.send(data);
              });
            })
        }
