...
class LocationController extends Controller
{
    ...
    public function store(StoreLocationRequest $request)
    {
        ...
        foreach ($request->input('gallery', []) as $file) {
            $location->addMedia(storage_path('tmp/uploads/' . basename($file)))->toMediaCollection('gallery');
        }
    ...
    public function update(UpdateLocationRequest $request, Location $location)
    {
        ...
        if (count($location->gallery) > 0) {
            foreach ($location->gallery as $media) {
                if (!in_array($media->file_name, $request->input('gallery', []))) {
                    $media->delete();
                }
            }
        }
        $media = $location->gallery->pluck('file_name')->toArray();
        foreach ($request->input('gallery', []) as $file) {
    ...
    public function show(Location $location)
    {
        ...
        $location->load('locationItineraries');
