...
class LocationApiController extends Controller
{
    ...
    public function store(StoreLocationRequest $request)
    {
        ...
        if ($request->input('gallery', false)) {
            $location->addMedia(storage_path('tmp/uploads/' . basename($request->input('gallery'))))->toMediaCollection('gallery');
        }
    ...
    public function update(UpdateLocationRequest $request, Location $location)
    {
        ...
        if ($request->input('gallery', false)) {
            if (!$location->gallery || $request->input('gallery') !== $location->gallery->file_name) {
                if ($location->gallery) {
                    $location->gallery->delete();
                }
                $location->addMedia(storage_path('tmp/uploads/' . basename($request->input('gallery'))))->toMediaCollection('gallery');
            }
